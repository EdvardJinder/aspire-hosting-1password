    ...
    protected sealed override async Task ExecuteCoreAsync(CancellationToken cancellationToken)
    {
        ...
        if (resolved is null || !isValid)
        {
            ...
            var message = (link, validationMessage) switch
            {
                ...
                (not null, null) => string.Format(CultureInfo.CurrentCulture, "Required command '{0}' was not found. See installation instructions for more details.", command),
                _ => string.Format(CultureInfo.CurrentCulture, "Required command '{0}' was not found on PATH or at a specified location.", command)
            };
